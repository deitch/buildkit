...
type ContainerImageResolverType int

const (
	ResolverTypeRegistry ContainerImageResolverType = iota
...
type SourceOpt struct {
	...
	Source        ContainerImageResolverType
...
func (is *Source) ID() string {
	if is.Source == ResolverTypeOCILayout {
...
func (is *Source) ResolveImageConfig(ctx context.Context, ref string, opt llb.ResolveImageConfigOpt, sm *session.Manager, g session.Group) (digest.Digest, []byte, error) {
	...
	case ResolverTypeOCILayout:
		...
		// unlike Resolve, which passes a SourceIdentifier which includes session ID, ResolveImageConfig does not.
		// GetOCILayoutResolver handles an invalid session ID by using .Any(), so this should be ok.
...
func (is *Source) Resolve(ctx context.Context, id source.Identifier, sm *session.Manager, vtx solver.Vertex) (source.SourceInstance, error) {
	...
	default:
		return nil, errors.Errorf("unknown source type: %v", is.Source)
	}
	pullerUtil = &pull.Puller{
